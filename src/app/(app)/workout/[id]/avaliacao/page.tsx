...
function getCelebrationImage(shareData: ShareData | null): string {
  if (!shareData) return '/pos treino/completo.jpg'
  if (shareData.streak >= 7) return '/pos treino/streak.jpg'
  if (shareData.rating === 5) return '/pos treino/recorde.jpg'
  return '/pos treino/completo.jpg'
}

function getCelebrationLabel(shareData: ShareData | null): { badge: string; title: string; subtitle: string } {
  if (!shareData) return { badge: 'Treino Concluído', title: 'Mais um no banco!', subtitle: 'Você está construindo algo real aqui. Orgulho total!' }
  if (shareData.streak >= 7) return {
    badge: `${shareData.streak} dias seguidos`,
    title: 'Sequência incrível!',
    subtitle: `${shareData.streak} treinos consecutivos. Isso é disciplina de verdade.`,
  }
  if (shareData.rating === 5) return {
    badge: 'Treino perfeito',
    title: 'Você arrasou!',
    subtitle: 'Nota máxima. Esse é o tipo de treino que muda tudo.',
  }
  return {
    badge: 'Treino Concluído',
    title: 'Mais um no banco!',
    subtitle: 'Você está construindo algo real aqui. Orgulho total!',
  }
}
...
function CelebrationScreen({ onClose, shareData }: { onClose: () => void; shareData: ShareData | null }) {
  ...
  const bgImage = getCelebrationImage(shareData)
  const { badge, title, subtitle } = getCelebrationLabel(shareData)
    ...
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-end justify-end">
      ...
      >
        ...
        <div
          ...
          style={{
            background: 'linear-gradient(to top, rgba(10,10,10,1) 0%, rgba(10,10,10,0.4) 55%, rgba(10,10,10,0.1) 100%)',
          }}
        />

        {/* Content */}
        <div className="relative z-10 flex flex-col items-center gap-4 px-6 pb-10 pt-16 text-center">
          <motion.div
          ...
          >
            <span
              ...
              style={{ color: '#FF8C00', backgroundColor: 'rgba(255,140,0,0.2)', border: '1px solid rgba(255,140,0,0.4)' }}
            >
              {badge}
            </span>
          </motion.div>
