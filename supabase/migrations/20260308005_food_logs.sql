-- Registro alimentar: tabela de logs de refeições
-- Ferramenta informativa (não prescrição de dieta) — Legal: similar ao MyFitnessPal/Yazio
...
-- Índice para queries diárias (padrão de uso: WHERE user_id AND data)
CREATE INDEX food_logs_user_data ON food_logs (user_id, data DESC);
